...
/**
 * 为了管理无限大的三维世界，系统不会创建一个巨大的 3D
 * 数组（会内存爆炸），而是把根体素的 3D 坐标 (x, y, z) 压缩成一个 1D
 * 的数字（哈希键值） 定义了哈希函数用到的两个大质数。P 用于乘法错位，N
 * 用于取模防溢出，极大降低哈希碰撞概率
 */
...
typedef struct PointToPlane {
    ...
    Eigen::Matrix<double, 6, 6>
        plane_var_;      // 该拟合平面的不确定度（协方差）包括平面中心点坐标和法向量的协方差
    ...
    float dis_to_plane_; // 激光点到平面的垂直距离（即我们需要最小化的 残差
                         // Residual）
} PointToPlane;
...
typedef struct VoxelPlane {
    ...
    Eigen::Matrix<double, 6, 6>
        plane_var_;             // 平面参数(法矢和距离)的协方差（FAST-LIVO2精细建模）
    ...
    bool is_plane_ =
        false;               // 这些点真的构成一个面吗？（通过判断最小特征值是否远小于其他两个）
...
public:
    ...
    std::vector<int>
        layer_init_num_; // 八叉树各层首次触发平面拟合所需的最少点数
    float
        quater_length_; // 当前体素块边长的四分之一，方便体素分裂时快速计算子体素中心坐标
    float
        planer_threshold_;      // 判定体素内点是否构成平面的阈值，如果最小特征值与中等特征值的比值大于这个阈值，就不构成平面
    int points_size_threshold_; // 体素内缓存的新点达到这个数量时，开始执行初步的几何检查和平面拟合
    int update_size_threshold_; // 当一个体素已经成功拟合出平面后，之后再落入这个体素的点不会立即引发平面的重新计算（PCA
                                // 极耗算力）。只有当新落入的点数（new_points_）累积达到这个阈值（比如5
                                // 个）时，系统才会做一次批量增量更新，修正平面的法向量和中心
    int max_points_num_;        // 一个体素节点内部最多允许保留的历史雷达点数量
    int max_layer_;             // 八叉树允许向下分裂的最大深度
    int new_points_;            // 记录自从上一次更新平面参数之后，又落入该体素的新的激光点数量
    bool init_octo_;            // 标记这个八叉树节点是否已经成功完成了首次平面拟合（PCA）
    bool update_enable_;        // 控制该节点是否允许接受新点并更新平面参数
    ...
    void init_plane(const std::vector<pointWithVar> &points,
                    VoxelPlane *plane); // 用 PCA 拟合平面
    void init_octo_tree();              // 初始化树
    void cut_octo_tree();               // 核心：分裂（切分）树！
    void UpdateOctoTree(
        const pointWithVar &pv); // 加入新点后增量更新八叉树节点的平面参数

    VoxelOctoTree *find_correspond(
        Eigen::Vector3d pw);                       // 给定一个查询点，向下深入寻找最底层的叶子节点
    VoxelOctoTree *Insert(const pointWithVar &pv); // 插入新点
};
...
public:
    ...
    VoxelMapConfig
        config_setting_;           // 存储体素地图的配置参数（如体素大小、最大层数等）
    ...
    std::unordered_map<VOXEL_LOCATION, VoxelOctoTree *>
        voxel_map_; // 使用哈希表存储体素地图，每个根体素位置对应一个八叉树节点
    ...
    PointCloudXYZI::Ptr
        feats_down_body_; // 在雷达坐标系下（Body Frame）降采样后的点云
    PointCloudXYZI::Ptr
        feats_down_world_; // 转换到世界坐标系下（World Frame）的点云

    M3D extR_; // 雷达与 IMU 之间的旋转外参
    V3D extT_; // 雷达与 IMU 之间的平移外参

    // 记录构建残差（搜近邻、算距离）和 ESIKF
    // 滤波器更新所花费的时间，并在终端输出，用于衡量系统是否满足 10Hz
    // 的实时性要求。
    ...
    /**
     * 系统会以 last_slide_position 为中心维护一个局部包围盒（比如
     * 100m×100m）的地图数据。 当当前位姿与 last_slide_position 之间的距离超过
     * sliding_thresh（比如 50m）时，触发地图滑动机制，（存疑） 把
     * last_slide_position 更新为当前位姿，并丢弃掉距离 last_slide_position 超过
     * half_map_size（比如 50m）的地图数据。
     * 这样就实现了一个以当前位姿为中心、半径为 half_map_size
     * 的局部地图，既保证了地图的实时性和局部性，又避免了内存占用过大。
     ...
     */
    V3D position_last_; // 记录上一次优化后的位姿位置，用于实现地图滑动（Map
                        // Sliding）。
    ...
    std::vector<M3D>
        cross_mat_list_; // 存储点坐标的反对称矩阵（Skew-symmetric Matrix）
    std::vector<M3D>
        body_cov_list_; // 存储每个激光点在雷达坐标系下的投影协方差矩阵
    std::vector<pointWithVar>
        pv_list_; // Point with Variance
                  // List，存储经过初步筛选、准备进行残差计算的点
    std::vector<PointToPlane>
        ptpl_list_; // Point-To-Plane List，存储成功在地图中找到匹配平面（Voxel
                    // Map 中的平面）的约束对
    ...
    // 触发激光点云的 ESIKF 序列更新，将先验状态 state_propagat
    // 结合地图观测进行修正
    ...
    void BuildVoxelMap();
    // 连接 LIO 和 VIO
    // 的桥梁。当视觉模块（VIO）进行“按需光线投射”拿到深度后，通过这个函数去体素地图里查询对应
    // 3D 坐标的颜色/灰度值，用于计算视觉光度误差。
    V3F RGBFromVoxel(const V3D &input_point);

    // 一帧数据匹配完成后（位姿确认为最优），调用此函数将当前帧的激光点**插入（Insert）**到哈希表中对应的
    // VoxelOctoTree 节点里
    ...
    // mapSliding 和 clearMemOutOfMap 配合工作，根据传入的 XYZ
    // 边界，把超出这个物理边界的哈希桶清空（erase），防止内存溢出
    void mapSliding();
    void clearMemOutOfMap(const int &x_max, const int &x_min, const int &y_max,
                          const int &y_min, const int &z_max, const int &z_min);

private:
    // 可视化，提取地图中每一个体素块拟合出的小平面，并通过 ROS MarkerArray 在
    // Rviz 里画出一个个带颜色的圆盘或方块
